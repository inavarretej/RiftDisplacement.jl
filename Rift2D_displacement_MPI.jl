...
include("Rift2DSetupFaultInclusion.jl") ## Need to create a new MPI Setup
...
function main(li, origin, phases_GMG, igg; nx=16, ny=16, figdir="figs2D", do_vtk =false)
    ...
    pt_stokes        = PTStokesCoeffs(li, di; ϵ=1e-5, Re=3*√10*π/2, r=0.5, CFL = 0.85 / √2.1) # Re=3π, r=0.7
    # ----------------------------------------------------
    ...
    xci_v = LinRange(minimum(xvi[1]), maximum(xvi[1]), nx_v), LinRange(minimum(xvi[2]), maximum(xvi[2]), ny_v)
    ...
    while it < 500 # run only for 5 Myrs
        ...
        @views τxx_nohalo .= Array(stokes.τ.xx[2:(end - 1), 2:(end - 1)]) # Copy data to CPU removing the halo
        @views τyy_nohalo .= Array(stokes.τ.yy[2:(end - 1), 2:(end - 1)]) # Copy data to CPU removing the halo
        ...
        @views εII_p_nohalo .= Array(stokes.ε.II[2:(end - 1), 2:(end - 1)]) # Copy data to CPU removing the halo
        ...
        gather!(τxx_nohalo, τxx_v)
        gather!(τyy_nohalo, τyy_v)
        gather!(η_vep_nohalo, η_vep_v)
        gather!(εII_nohalo, εII_v)
        gather!(εII_p_nohalo, εII_p_v)
        gather!(P_nohalo,P_v)
